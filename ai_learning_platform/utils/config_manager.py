...
class ConfigManager:
    ...
    def load_firebase_config(self) -> Optional[str]:
        """
        Loads the Firebase credentials path from the config.

        Returns:
            str: The path to the Firebase credentials JSON file, or None if not found.
        """
        config = self.get_config()
        active_profile = config.get("active_profile", "default")
        firebase_config = config.get("profiles", {}).get(active_profile, {}).get("firebase", {})
        return firebase_config.get("credentials_path")

    def load_uid(self) -> Optional[str]:
        """
        Loads the UID from the config.
        ...
        active_profile = config.get("active_profile", "default")
        firebase_config = config.get("profiles", {}).get(active_profile, {}).get("firebase", {})
        return firebase_config.get("uid")
